...
const Leaderboard = () => {
  ...
  return (
    <div className="bg-green-600 min-h-screen flex flex-col items-center justify-center">
      ...
      <table className="mt-4 border-collapse border border-green-900">
        <thead>
          <tr>
            <th className="bg-green-800 text-white py-2 px-4">Email</th>
            <th className="bg-green-800 text-white py-2 px-4">Score</th>
          </tr>
        </thead>
        <tbody>
          {leaderboardData.map((entry, index) => (
            <tr key={index} className={index % 2 === 0 ? 'bg-green-700' : 'bg-green-700'}>
              <td className="border border-green-900 py-2 px-4 text-white">{entry.email}</td>
              <td className="border border-green-900 py-2 px-4 text-white">{entry.score}</td>
            </tr>
          ))}
        </tbody>
      </table>
